...
public class Main_window extends javax.swing.JFrame {
    ...
    private void initializeClient() {
        try {
            ...
            JOptionPane.showMessageDialog(this, "Соединение с сервером установлено", "Успех", JOptionPane.INFORMATION_MESSAGE);
        } catch (IOException e) {
            JOptionPane.showMessageDialog(this, "Не удалось подключиться к серверу: " + e.getMessage(), "Ошибка", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private class ParallelIntegral {
        ...
        private Client client; // Добавляем ссылку на клиент

        public ParallelIntegral(double low, double high, double step,
                                int selectedRow, DefaultTableModel model, Client client) {
            this.low = low;
            this.high = high;
            this.step = step;
        ...
        public void calculate() {
            ...
            Thread secondThread = new Thread(() -> {
                try {
                    ...
                    String response = client.Help(mid, high, step);

                    if (response.startsWith("RESULT:")) {
                        System.out.println(response);
                        String[] parts = response.split(":");
                        secondResult.value = Double.parseDouble(parts[1]);
                    } else {
    ...
    private void Button_rezultMouseClicked(java.awt.event.MouseEvent evt) {
        ...
        if (selectedRow != -1) {
            ...
            System.out.println("Перед созданием потока"); // Логирование 1
            new Thread(() -> {
                try {
                    System.out.println("Поток начал выполнение");
                    new ParallelIntegral(low, high, step, selectedRow, model, client).calculate();
                } catch (Throwable t) {
                    System.err.println("Фатальная ошибка в потоке:");
                    t.printStackTrace();
                }
            }).start();
